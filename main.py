...
if use_spear:
    ...
    # Get structures and parse info as before
    paths = paths_dic(paths_file)
    struct_info_dic, struct_info_df = xml_parser(paths['structs_info'])
    
    # Get and preprocess structures as before
    ...
    # Now run SPEAR-specific pipeline instead of the original flow
    frtmalign_module.run_spear_pipeline(
        paths['clean_dir'], 
        paths['frtmalign_dir'], 
        paths['frtmalign'],
        paths['pdb_dir'],
        paths['clean_dir'],
        struct_info_df,
        paths['vdw_radius_file'],
        paths['hole_reference_pore_point']
    )
else:
    # set working directory, output directories
    # identify files containing necessary information, including paths to program files (frtmalign, HOLE, pyali), information for each structure
